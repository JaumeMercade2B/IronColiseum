...
public class TorretaBehaviour : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (canShoot == true)
        {
            torreta.transform.LookAt(playerTrans);
        }
        cadency += Time.deltaTime;

        if (canShoot && cadency >= waitCadency && canShoot == true)
        {
            ...
            if (Physics.Raycast(transform.position, transform.forward, out hit, distance)) //Aquí diem si el raig ha xocat amb quelcom
            {
